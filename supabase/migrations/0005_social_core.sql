...
--
-- Safe to re-run: every step below is guarded so a partially-applied previous
-- attempt won't error out on re-execution.
...
begin
  if exists (
    ...
    where table_name = 'post_comments' and column_name = 'author_id'
  ) then
...
begin
  if exists (
    select 1 from pg_constraint where conname = 'post_comments_author_id_fkey'
  ) then
...
drop policy if exists "post_comments write" on post_comments;
drop policy if exists "post_comments delete own" on post_comments;
...
-- ---------------------------------------------------------------
create unique index if not exists rig_items_user_gear_uidx on rig_items(user_id, gear_id) where gear_id is not null;
create unique index if not exists rig_items_user_custom_gear_uidx on rig_items(user_id, custom_gear_id) where custom_gear_id is not null;

-- ---------------------------------------------------------------
-- spaces: support user-created spaces alongside the fixed core catalog
-- ---------------------------------------------------------------
alter table spaces add column if not exists is_public boolean not null default true;
alter table spaces add column if not exists created_by uuid references profiles(id) on delete set null;

drop policy if exists "catalog read" on spaces;
...
create or replace function create_user_space(space_name text, space_is_public boolean default true)
returns text as $$
declare
